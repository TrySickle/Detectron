...
def main(args):
    ...
    for i, im_name in enumerate(im_list):
        ...
        print(cls_boxes_np.shape)
        print('cls_boxes_np', cls_boxes_np)
        cls_boxes_prob = cls_boxes_np[1][:,4]
        print('cls_boxes_prob', cls_boxes_prob)
        idx_max_prob = np.argmax(cls_boxes_prob)

        cls_keyps_max_prob = cls_keyps[1][idx_max_prob]
        print('cls_keyps', cls_keyps)
        print('cls_keyps_max_prob', cls_keyps_max_prob)
        pose_x_y_prob_after_softmax = cls_keyps_max_prob[[0,1,3]]
        print(np.transpose(pose_x_y_prob_after_softmax))
        print(np.transpose(pose_x_y_prob_after_softmax).shape)
